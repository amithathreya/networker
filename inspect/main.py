...
from scapy.all import sniff, IP, IPv6, TCP, UDP, ICMP, Ether, Raw
...
def packet_handler(packet):
    ...
    info["timestamp_utc"] = datetime.fromtimestamp(packet.time, tz=timezone.utc).isoformat()
...
def print_flow_summary():
    ...
    for flow_key, pkts in sorted(flows.items(), key=lambda x: min(p["timestamp_epoch"] for p in x[1])):
        if not pkts:
            continue
